...
const Domestic = () => {
  console.log("Domestic Component Loaded");

  const [matches, setMatches] = useState([]);
  const API_URL =
    "https://cricbuzz-cricket2.p.rapidapi.com/schedule/v1/domestic?lastTime=1729555200000";
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(API_URL, {
          method: "GET",
          headers: {
            "X-RapidAPI-Key": import.meta.env.VITE_RAPIDAPI_KEY_DOMESTIC,
            "X-RapidAPI-Host": import.meta.env.VITE_RAPIDAPI_HOST_DOMESTIC,
          },
        });
        ...
        console.error("Error fetching Domestic schedule:", error);
        setMatches([]);
      }
    };
  ...
  return (
    <div className="bg-gray-850 p-4 rounded-lg shadow-lg">
      ...
      {matches.length > 0 ? (
        ...
        <p className="text-center text-gray-400">No Domestic matches available.</p>
      )}
    </div>
  );
};
