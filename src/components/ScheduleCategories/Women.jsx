...
const Women = () => {
  ...
  const API_URL =
    "https://cricbuzz-cricket2.p.rapidapi.com/schedule/v1/women?lastTime=1729641600000";
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(API_URL, {
          method: "GET",
          headers: {
            "X-RapidAPI-Key": import.meta.env.VITE_RAPIDAPI_KEY_WOMEN,
            "X-RapidAPI-Host": import.meta.env.VITE_RAPIDAPI_HOST_WOMEN,
          },
        });
        ...
        console.error("Error fetching Women schedule:", error);
        setMatches([]);
      }
    };
  ...
  return (
    <div className="bg-gray-850 p-4 rounded-lg shadow-lg">
      ...
      {matches.length > 0 ? (
        ...
        <p className="text-center text-gray-400">No Women matches available.</p>
      )}
    </div>
  );
};
